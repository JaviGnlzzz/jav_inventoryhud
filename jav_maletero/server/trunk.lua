...
function loadInvent(plate)
    local result = MySQL.Sync.fetchAll("SELECT * FROM trunk_inventory WHERE plate = @plate", {["@plate"] = plate})
    if #result ~= 0 then
        for i = 1, #result, 1 do
            local plate = result[i].plate
            local owned = result[i].owned
...
function getOwnedVehicule(plate)
  if plate == nil then
    ...
    if not found then
        ...
        if #result > 0 then found = true end
    end
    return found
end
...
function MakeDataStore(plate)
  ...
  local dataStore = CreateDataStore(plate, owned, data)
  SharedDataStores[plate] = dataStore
  MySQL.Async.execute("INSERT INTO trunk_inventory(plate,data,owned) VALUES (@plate,'{}',@owned)", {['@plate'] = plate, ['@owned'] = owned})
  loadInvent(plate)
  print(json.encode(data))
end

function GetSharedDataStore(plate)
    if SharedDataStores[plate] == nil then MakeDataStore(plate) end
    return SharedDataStores[plate]
end

AddEventHandler("esx_trunk:getSharedDataStore", function(plate, cb) 
  if plate == nil or plate == '' or plate == ' ' or cb == nil then
    return
  end
