      ...
      if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
        ...
        if vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(closecar, -1) ~= PlayerPedId() then
          ...
          if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
            ...
            --ESX.ShowNotification('No tienes ningun coche ~r~cerca tuya!')
            end
          end
        else
          --ESX.ShowNotification('No tienes ningun coche ~r~cerca tuya!')
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
  ESX.TriggerServerCallback(
    ...
    function(inventory)
      ...
      TriggerEvent("jav_inventoryhud:openTrunkInventory", data, inventory.blackMoney, inventory.items, inventory.weapons)
    end,
    plate
  )
end
