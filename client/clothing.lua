...
local Extras = {
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, Male = 102, Female = 15},
...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then ESX.ShowNotification("Esta ped no admite esto.") return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then
		...
		ESX.ShowNotification("No tienes ninguna prenda.") return
	else
		if not LastEquipped[which] then
	...
	ESX.ShowNotification("No tienes ninguna prenda") return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		ESX.ShowNotification("No tienes nada que quitarte.") return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then ESX.ShowNotification("Este modelo no soporta esta opción.") return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		ESX.ShowNotification("No parece que haya ninguna variante de esto") return false
	end
end
